...
float caudal_setpoint1 = 0.0;
float caudal_setpoint2 = 0.0;
...
float temp_inicial_setpoint = 0.0;  // Temperatura inicial deseada
bool temp_inicial_alcanzada = false;  // Bandera de temperatura alcanzada
bool modo_test = false;  // Modo test (bypass de temperatura)
bool sistema_iniciado = false;  // Sistema en operación
...
void VerificarTemperaturaInicial(float temp_actual) {
  // Si ya está iniciado o en modo test, no hacer nada
  if (sistema_iniciado || modo_test) {
    return;
  }
  
  // Si no hay setpoint configurado, no iniciar
  if (temp_inicial_setpoint == 0) {
    return;
  }
  
  // Verificar si alcanzó la temperatura
  if (temp_actual >= temp_inicial_setpoint && !temp_inicial_alcanzada) {
    temp_inicial_alcanzada = true;
    sistema_iniciado = true;
    Serial.println("*** TEMPERATURA INICIAL ALCANZADA - SISTEMA INICIADO ***");
  }
}
...
void ProcesarComandoSerial() {
  if (Serial.available()) {
    ...
    // Comando: TINIT:valor = temperatura inicial
    else if (comando.startsWith("TINIT:")) {
      float valor = comando.substring(6).toFloat();
      temp_inicial_setpoint = valor;
      temp_inicial_alcanzada = false;
      sistema_iniciado = false;
      Serial.print("Temperatura inicial configurada: ");
      Serial.print(valor);
      Serial.println(" C");
    }
    
    // Comando: TEST = modo test (bypass temperatura)
    else if (comando == "TEST") {
      modo_test = true;
      sistema_iniciado = true;
      Serial.println("MODO TEST ACTIVADO - Sistema iniciado");
    }
    
    // Comando: NORMAL = salir de modo test
    else if (comando == "NORMAL") {
      modo_test = false;
      sistema_iniciado = false;
      temp_inicial_alcanzada = false;
      Serial.println("MODO NORMAL - Sistema detenido");
    }
    
    // Comando: START = iniciar sistema (solo si temp alcanzada o modo test)
    else if (comando == "START") {
      if (modo_test || temp_inicial_alcanzada) {
        sistema_iniciado = true;
        Serial.println("Sistema INICIADO manualmente");
      } else {
        Serial.println("ERROR: Temperatura inicial no alcanzada");
      }
    }
    
    // Comando: STOP = detener sistema
    ...
    // Comando: STATUS = mostrar estado del sistema
    else if (comando == "STATUS") {
      Serial.println("=== ESTADO DEL SISTEMA ===");
      Serial.print("Temperatura setpoint: ");
      Serial.println(temp_inicial_setpoint);
      Serial.print("Temperatura alcanzada: ");
      Serial.println(temp_inicial_alcanzada ? "SI" : "NO");
      Serial.print("Modo test: ");
      Serial.println(modo_test ? "SI" : "NO");
      Serial.print("Sistema iniciado: ");
      Serial.println(sistema_iniciado ? "SI" : "NO");
      Serial.println("========================");
    }
  }
}
...
void MostrarDatos(float t1, float t2, float t3) {
  ...
  } else if (sistema_iniciado) {
    Serial.println("EN OPERACION");
  } else if (temp_inicial_setpoint > 0) {
    Serial.print("ESPERANDO TEMP (");
    Serial.print(temp_inicial_setpoint, 1);
    Serial.println(" C)");
  } else {
...
void setup() {
  Serial.begin(115200);
  Serial.println("Sistema de control de caudal con temperatura inicial");
  ...
  Serial.println("  TINIT:X = temperatura inicial (C)");
  Serial.println("  TEST = activar modo test");
  Serial.println("  NORMAL = modo normal");
...
void loop() {
  ...
  if (tiempoActual - tiempoAnterior >= intervaloMuestreo) {
    ...
    VerificarTemperaturaInicial(t1);
    
    // Solo operar bombas si el sistema está iniciado
    if (sistema_iniciado) {
      ...
      // Sistema detenido, bombas apagadas
      analogWrite(RPWM1, 0);
      analogWrite(RPWM2, 0);
      
      // Aún así calcular caudales para monitoreo
      CalcularCaudalesYVolumenes(dt_s);
    }
