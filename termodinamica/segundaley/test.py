...
class MonitorArduino:
    def __init__(self, root):
        ...
        # Variables de temperatura inicial
        self.temp_inicial = tk.StringVar(value="0")
        self.estado_sistema = tk.StringVar(value="DETENIDO")
        
        # Estados: IDLE, WAITING_FOR_TEMPERATURE, RUNNING
        self.estado_actual = "IDLE"
        self.temp_objetivo = 0.0  # Temperatura objetivo configurada
    ...
    def iniciar_sesion_logging(self):
        """Inicia una nueva sesión de logging con archivos CSV y LOG"""
        self.session_start = datetime.now()
        timestamp = self.session_start.strftime("%Y%m%d_%H%M%S")
        
        # Archivo CSV para datos
        csv_filename = f"logs/datos_{timestamp}.csv"
        ...
        # Archivo LOG para eventos
        log_filename = f"logs/eventos_{timestamp}.log"
        self.log_file = open(log_filename, 'w')
        
        self.escribir_log(f"=== SESIÓN INICIADA ===")
        self.escribir_log(f"Archivo de datos: {csv_filename}")
        self.escribir_log(f"Archivo de eventos: {log_filename}")
        
        print(f"Logging iniciado: {csv_filename}")
    
    def detener_sesion_logging(self):
        """Cierra los archivos de logging"""
        if self.csv_file:
            self.csv_file.close()
            self.csv_file = None
            self.csv_writer = None
        ...
        if self.log_file:
            self.escribir_log("=== SESIÓN FINALIZADA ===")
            self.log_file.close()
            self.log_file = None
    ...
    def guardar_datos_csv(self, datos):
        ...
        if not self.csv_writer or not self.session_start:
            return
        
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
        tiempo_relativo = (datetime.now() - self.session_start).total_seconds()
        
        try:
            self.csv_writer.writerow([
                timestamp,
                f"{tiempo_relativo:.3f}",
                datos.get('sistema', '--'),
                datos.get('temp1', '--'),
                datos.get('temp2', '--'),
                datos.get('temp3', '--'),
            ...
            self.escribir_log(f"ERROR CSV: {e}")
    ...
    def crear_interfaz(self):
        ...
        # Estado del sistema
        tk.Label(frame_top, text="Estado:", bg='#2b2b2b', fg='white',
                font=('Arial', 10)).pack(side='left', padx=(40, 5))
        tk.Label(frame_top, textvariable=self.estado_sistema, bg='#2b2b2b', 
                fg='#ffc107', font=('Arial', 11, 'bold')).pack(side='left')
        ...
        # ========== COLUMNA 1: CONTROL DE INICIO Y BOMBAS ==========
        ...
        # --- CONTROL DE INICIO ---
        frame_inicio = tk.LabelFrame(frame_col1, text="CONTROL DE INICIO", 
                                     bg='#363636', fg='white', 
                                     font=('Arial', 12, 'bold'), padx=15, pady=10)
        frame_inicio.pack(fill='x', pady=(0, 10))
        
        # Temperatura inicial
        temp_frame = tk.Frame(frame_inicio, bg='#363636')
        temp_frame.pack(fill='x', pady=5)
        
        tk.Label(temp_frame, text="Temperatura Inicial (°C):", bg='#363636', fg='white',
                font=('Arial', 10, 'bold')).pack(anchor='w')
        
        spinbox_frame = tk.Frame(temp_frame, bg='#363636')
        spinbox_frame.pack(fill='x', pady=5)
        
        self.spin_temp = tk.Spinbox(spinbox_frame, from_=0, to=100, increment=5,
                                   textvariable=self.temp_inicial,
                                   font=('Arial', 14, 'bold'), width=8,
                                   state='disabled', justify='center')
        self.spin_temp.pack(side='left')
        
        self.btn_set_temp = tk.Button(spinbox_frame, text="Establecer", 
                                      command=self.establecer_temp_inicial,
                                      bg='#17a2b8', fg='white', 
                                      font=('Arial', 10, 'bold'),
                                      state='disabled', width=12)
        self.btn_set_temp.pack(side='left', padx=10)
        
        tk.Label(temp_frame, text="El sistema iniciará al alcanzar esta temperatura",
                bg='#363636', fg='#aaa', font=('Arial', 8), wraplength=380,
                justify='left').pack(anchor='w', pady=2)
        
        ttk.Separator(frame_inicio, orient='horizontal').pack(fill='x', pady=8)
        
        # Botones de control
        botones_frame = tk.Frame(frame_inicio, bg='#363636')
        botones_frame.pack(fill='x')
        
        self.btn_start = tk.Button(botones_frame, text="▶ INICIAR",
        ...
        self.btn_stop = tk.Button(botones_frame, text="■ DETENER",
    ...
    def conectar(self):
        ...
        try:
            ...
            # Iniciar logging
            self.iniciar_sesion_logging()
            self.escribir_log(f"Conectado a puerto: {puerto}")
            ...
            self.spin_temp.config(state='normal')
            ...
            self.escribir_log(f"ERROR CONEXIÓN: {str(e)}")
            messagebox.showerror("Error de Conexión", f"No se pudo conectar:\n{str(e)}")
    
    def desconectar(self):
        self.escribir_log("Desconectando...")
        self.is_monitoring = False
        time.sleep(0.5)
        ...
        self.spin_temp.config(state='disabled')
        self.btn_set_temp.config(state='disabled')
        self.btn_start.config(state='disabled')
        self.btn_stop.config(state='disabled')
    
    def establecer_temp_inicial(self):
        if self.is_connected:
            temp = self.temp_inicial.get()
            try:
                self.temp_objetivo = float(temp)
                self.escribir_log(f"Temperatura objetivo establecida: {self.temp_objetivo}°C")
                messagebox.showinfo("Temperatura Establecida", 
                                   f"Temperatura objetivo: {self.temp_objetivo}°C\n\n" +
                                   "El sistema iniciará al presionar INICIAR.")
            except ValueError:
    ...
    def iniciar_sistema(self):
        ...
        try:
            ...
            temp_actual_str = self.temp1.get()
            if temp_actual_str == "--":
                messagebox.showwarning("Advertencia", 
                                     "Esperando lectura de temperatura...")
                return
            
            temp_actual = float(temp_actual_str)
            
            # Verificar si ya alcanzó la temperatura
            if temp_actual >= self.temp_objetivo:
                # Ya alcanzó, ir directo a RUNNING
                self.estado_actual = "RUNNING"
                self.estado_sistema.set("OPERANDO")
                self.serial_connection.write(b"START\n")
                self.escribir_log(f"Sistema INICIADO - Temperatura ya alcanzada ({temp_actual:.2f}°C >= {self.temp_objetivo}°C)")
            else:
                # No alcanzó, ir a WAITING_FOR_TEMPERATURE
                self.estado_actual = "WAITING_FOR_TEMPERATURE"
                self.estado_sistema.set(f"ESPERANDO {self.temp_objetivo}°C")
                self.escribir_log(f"Sistema en espera - Temp actual: {temp_actual:.2f}°C, Objetivo: {self.temp_objetivo}°C")
            ...
            self.escribir_log(f"ERROR al iniciar: {str(e)}")
    ...
    def detener_sistema(self):
        if self.is_connected:
            respuesta = messagebox.askyesno("Confirmar", 
                                           "¿Detener el sistema?\n\n" +
                                           "Las bombas se apagarán.")
            if respuesta:
                try:
                    # Volver a IDLE
                    self.estado_actual = "IDLE"
                    self.estado_sistema.set("DETENIDO")
                    ...
                    self.escribir_log("Sistema DETENIDO por usuario")
                    ...
                    self.escribir_log(f"ERROR al detener: {str(e)}")
    ...
    def actualizar_setpoint1(self, valor):
        ...
        if self.is_connected:
            ...
            try:
                ...
                self.escribir_log(f"Setpoint Bomba 1: {valor_float} L/min")
            except Exception as e:
    ...
    def actualizar_setpoint2(self, valor):
        ...
        if self.is_connected:
            ...
            try:
                self.serial_connection.write(comando.encode())
                self.escribir_log(f"Setpoint Bomba 2: {valor_float} L/min")
            except Exception as e:
                print(f"Error enviando setpoint 2: {e}")
    
    def detener_todo(self):
        if self.is_connected:
            respuesta = messagebox.askyesno("Emergencia", 
                                           "¿DETENER TODO EL SISTEMA?\n\n" +
                                           "Todas las bombas se apagarán inmediatamente.")
            if respuesta:
                try:
                    self.serial_connection.write(b"OFF\n")
                    self.slider_setpoint1.set(0)
                    self.slider_setpoint2.set(0)
                    self.escribir_log("!!! PARADA DE EMERGENCIA !!!")
                except Exception as e:
                    self.escribir_log(f"ERROR en parada de emergencia: {str(e)}")
                    messagebox.showerror("Error", f"No se pudo detener:\n{str(e)}")
    ...
    def reset_volumenes(self):
        if self.is_connected and self.serial_connection:
            try:
                ...
                self.escribir_log("Volúmenes reiniciados")
                messagebox.showinfo("Reset", "Volúmenes reiniciados")
            except Exception as e:
                self.escribir_log(f"ERROR al resetear volúmenes: {str(e)}")
                ...
                if self.serial_connection.in_waiting:
                    ...
                    if "Sistema:" in linea:
                        try:
                            ...
                            datos_actuales['sistema'] = estado
                        except:
                            pass
                    
                    elif "Sensor 1 (pin11):" in linea:
                        try:
                    ...
                    if "========================================" in linea and datos_actuales:
                        ...
                        self.guardar_datos_csv(datos_actuales)
                        datos_actuales = {}
                
                # Verificar transición de estado WAITING -> RUNNING
                if self.estado_actual == "WAITING_FOR_TEMPERATURE":
                    temp_actual_str = self.temp1.get()
                    if temp_actual_str != "--":
                        try:
                            temp_actual = float(temp_actual_str)
                            if temp_actual >= self.temp_objetivo:
                                # Temperatura alcanzada, cambiar a RUNNING
                                self.estado_actual = "RUNNING"
                                self.estado_sistema.set("OPERANDO")
                                self.serial_connection.write(b"START\n")
                                self.escribir_log(f"!!! TEMPERATURA OBJETIVO ALCANZADA ({temp_actual:.2f}°C) - BOMBAS ENCENDIDAS !!!")
                        except:
                            pass
                        
            except Exception as e:
                error_msg = f"Error leyendo datos: {e}"
                print(error_msg)
                self.escribir_log(f"ERROR LECTURA: {e}")
                time.sleep(0.1)
    
    def actualizar_display(self, datos):
        if 'sistema' in datos:
            # No sobrescribir el estado del sistema si estamos en WAITING
            if self.estado_actual == "IDLE" or self.estado_actual == "RUNNING":
                self.estado_sistema.set(datos['sistema'])
